...
class Test_Domain_Scaler(unittest.TestCase):
	...
	def test_domain_scaler(self):
		ds = DomainScaler(dim=3,input_range=[(1.,3.),(1.,3.),(1.,3.)])
	...
	def test_nonuniform_input_range(self):
		ds = DomainScaler(dim=3,input_range=[(1.,3.),(1.,4.),(1.,5.)])
		Y = np.array([[1,1,1], [3.,4.,5.]])
		Yt = ds.fit_transform(Y)
		assert np.mean(Yt[0]) == -1.0, "transform is broken"
		assert np.mean(Yt[1]) == 1.0, "transform is broken" 
		error = np.linalg.norm(ds.inverse_transform(Yt) - Y)
	...
	def test_nonuniform_input_range2(self):
		ds = DomainScaler(dim=3,input_range=[(1.,3.),(1.1,4.),(1.2,5.)])
	...
	def test_nondefault_input_output_range(self):
		ds = DomainScaler(dim=3,input_range=[(1.,3.),(1.1,4.),(1.2,5.)],output_range=(10,11.))
		Y = np.array([[1,1.1,1.2], [3.,4.,5.]])
		Yt = ds.fit_transform(Y)
...
class Test_Log_Scaler(unittest.TestCase):
	...
	def test_log_transform(self):
		...
		logY = np.log(Y)
		self.LT.fit(Y)
		assert np.linalg.norm(self.LT.transform(Y) - logY) < 1e-15, ".transform method is not working. "
	def test_log_inverse_transform(self):
		Y = self.Y
		logY = np.log(Y)
		self.LT.fit(Y)
		Yhat = self.LT.transform(Y)
		assert np.linalg.norm(self.LT.inverse_transform(Yhat) - Y) < 1e-15, ".transform method is not working. "
...
class Test_Pipeline(unittest.TestCase):
	...
	def test_gen_pipeline_log_scale_pca(self):
		...
		pipe = gen_scale_pipeline(log=True,scale=True,pca=True,n_components=ncomp,svd_solver='auto')
		Y = self.Y
		pipe.fit(Y)
		Yhat = pipe.transform(Y)
		Yr = pipe.inverse_transform(Yhat)
		...
		assert rmse.mean() < 1e-16, "PCA should be exact, but isn't. Something is wrong."
...
class Test_PCA_Target_Transform(unittest.TestCase):
	...
	def setUpClass(self):
		self.rn = np.random.RandomState(399123)
		self.K = 8
		self.Y = make_low_rank_matrix(n_samples=100,n_features=75,effective_rank=self.K,tail_strength=.01,random_state=self.rn)
	...
	def test_pca_auto(self):
		...
		cutoff = 1e-2
		pca = PCATargetTransform(n_components='auto',cutoff=cutoff)
		pca.fit(Y)
