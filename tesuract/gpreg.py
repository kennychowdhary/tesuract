...
def nplml(theta,X,y,kernel,nugget=1e-10):
	...
	log_likelihood_dims -= .5*np.log(np.linalg.det(K))
...
def minimize_call(xstart,objfunc,bounds,factr,disp=5):
	...
	'''
	res = fmin_l_bfgs_b(objfunc,xstart,approx_grad=True,bounds=bounds,factr=factr,pgtol=1e-13,disp=disp)
	return res
...
import emcee
class GPReg:
	def __init__(self,kernel,order=2,poly_alg='monomials',input_range=None,alpha=None,n_restarts_optimizer=0,random_state=None):
		self.kernel = kernel
		self.order = order
	...
	def _compile(self,X,y):
		...
		self.y = y.copy()
		self.X = X.copy()
		if self.y.ndim == 1: self.y = self.y[:,np.newaxis]
		assert self.X.shape[0] == self.y.shape[0], "shape mistmatch."
		if self.order == 0:
			self.Xhat = self.X
		elif self.order > 0:
			if self.poly_alg == "Legendre":
				self.pce = tesuract.PCEBuilder(order=self.order,input_range=self.input_range) #store_phi=True to keep in the pce object
		...
		self.feature_labels = ["theta_"+str(t+1) for t in range(self.theta_size)] + ["coef_"+str(c) for c in range(self.coef_size)]
	...
	def log_marginal_likelihood(self,theta,coef=None,nugget=1e-6,eval_gradient=False,coef_bounds=(-2.,2.)):
		if coef is None:
			self.coef = np.zeros(self.coef_size)
			self.y_minus_mu = self.y
		else:
			assert len(coef) == self.coef_size, "coefficient array is not the corrent size."
		...
		assert self.compile_flag == True, "Must compile first to precompute polynomial transform and save X,y data."
		assert len(theta) == self.theta_size, "theta wrong length."
		loglike = clml(theta,self.X,self.y_minus_mu,self.kernel,nugget=nugget,eval_gradient=eval_gradient)
		return loglike
	# test optimize
	...
	def log_marginal_likelihood2(self,theta,nugget=1e-8,return_coef=False):
		# log marginal likelihood where coef(theta) is computed analytically
		# This is all to compute the coef as a function of theta params
		...
		#### new method with automatic regularization
		# Xtilde = np.linalg.solve(L,H)
		# ytilde = np.linalg.solve(L,y)
	...
	def fit2(self,X,y):
		self._compile(X,y)
		opt_args = {'factr':1e7,'disp':25}
		myfun = partial(minimize_call, objfunc=self._objfunc2,bounds=self.theta_bounds,**opt_args)
		...
		logl, self.coef_ = self.log_marginal_likelihood2(self.theta_,return_coef=True)#self.xmap_[self.theta_size:]	# separate pce coef
		return self
	def fit(self,X,y):
		self._compile(X,y)
		opt_args = {'factr':1e2,'disp':25}
		myfun = partial(minimize_call, objfunc=self._objfunc,bounds=self.all_bounds,**opt_args)
		nsolves = self.n_restarts_optimizer + 1
		# xstarts = [np.zeros(len(self.all_bounds)) for i in range(nsolves)]
		if self.random_state is None:
			self.random_state = np.random.RandomState(997)
		##########
		start_bounds = self.theta_bounds + [(-100,100.) for c in range(self.coef_size)]
		xstarts = random_sample_within_bounds(nsolves,start_bounds)
		# add solver for pce coef only to improve problem of poor fit when data not scaled. 
		##########
		# xstarts = [2*self.random_state.rand(len(self.all_bounds))-1 for i in range(nsolves)]
	...
	def fit_bayesian(self,X,y):
		...
		return self
	def predict_bayes(self,X,return_pce_only=False,return_std=False,return_cov=False,return_samples=None):
	...
	def predict(self,X,return_pce_only=False,return_std=False,return_cov=False,return_samples=None):
		# plot the test prediction using the GP PCE model
...
def plot_samples(Y,x=None,ax=None,q=[.005,.5,.995],label=None,color=None,alpha=.5,use_std=False,fill=True):
	...
	if fill is True:
		if q is None:
			ax.fill_between(x, mu - 2*np.sqrt(var), mu + 2*np.sqrt(var),color=color, alpha=alpha,label=label)
			ax.plot(x, mu, color=color, lw=1.5, alpha=alpha)
		else:
			Q = np.quantile(Y,q=q,axis=0)
			ax.fill_between(x, Q[0], Q[2],color=color, alpha=alpha,label=label)
			ax.plot(x, Q[1], color=color, lw=1.5, alpha=alpha)
	else:
		if q is None:
			ax.plot(x, mu - 2*np.sqrt(var),'--',color=color, alpha=alpha,label=label)
			ax.plot(x, mu + 2*np.sqrt(var),'--',color=color, alpha=alpha)
			# ax.plot(x, mu, color=color, lw=1.5, alpha=alpha)
		else:
			Q = np.quantile(Y,q=q,axis=0)
			ax.fill_between(x, Q[0], Q[2],color=color, alpha=alpha,label=label)
