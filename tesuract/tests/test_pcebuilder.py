...
class TestPCEBuilderOnArbDomain(unittest.TestCase):
    ...
    def test_full_domain_scaling(self):
        ...
        scaler = tesuract.preprocessing.DomainScaler(
            dim=d, input_range=ibounds, output_range=(-1, 1)
        )
    ...
    def test_pcebuild_with_input_range(self):
        ...
        assert (
            mse(Xhat_ref, Xhat) <= 1e-16
        ), "mismatch between expected poly transform and actual poly transform"
    ...
    def test_pcebuild_polyeval_with_input_range(self):
        ...
        assert np.all(
            ypred == 1
        ), "vector must be all ones. Something may have gone wrong with polyeval and input bounds."
...
class TestPCEBuilder(unittest.TestCase):
    ...
    def test_pcebuilder_customM_as_float(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert (
                p2.mindex.dtype == "int"
            ), "Custom multiindex is not saved as an integer type."
    ...
    def test_fit_transform(self):
        ...
        assert (
            Xhat.shape[1] == p.mindex.shape[0]
        ), "Mistmatch between number of columns of Xhat (Vandermonde) and terms in multiindex."
    ...
    def test_fit_transform_w_normalized(self):
        ...
        assert np.abs(int_P0 - 1.0) <= 1e-13, "First term should integrate to 1"
    ...
    def test_pcebuilder_poleval_w_custom_coef(self):
        ...
        coef = np.array([0.0, -0.25, 1.0 / 3, np.sqrt(2)])  # ,-.05*np.sqrt(2)])
    ...
    def test_polyeval_w_variable_coef_and_fixed_X(self):
        ...
        ctest = np.zeros(len(c_true))
        ctest[2] = 1.0
        ypred2 = p.polyeval(c=ctest)
        assert (
            np.sum(np.abs(ypred2 - X[:, 1])) <= 1e-15
        ), "polyeval with fixed X is not working right."
    ...
    def test_polyeval_custom_M_and_custom_coef(self):
        ...
        assert (
            mse(p.polyeval(X), y) <= 1e-15
        ), "polyeval not returning accurate prediction."
    ...
    def test_sobol_sensitivity_w_normalization(self):
        ...
        assert (
            mse(s, [1.0, 0.2]) == 0
        ), "sobol indices for normalized case is  not right."
    ...
    def test_timing_for_repeated_transform(self):
        ...
        assert (
            pceb._mindex_compute_count_ == 1
        ), "mindex is being computed more than once or none at all."
...
class TestJointSobol(unittest.TestCase):
    ...
    def test_joint_sensitivity_2D_3rd(self):
        ...
        coef_sum = np.sum(
            [
                pce.normsq[i] * pce.coef[i] ** 2
                for i in range(len(pce.coef))
                if ((pce.mindex[i, 0] > 0) and (pce.mindex[i, 1] > 0))
            ]
        )
